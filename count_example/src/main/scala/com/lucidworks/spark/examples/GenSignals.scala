...
val dataDir = "queries_parquet"
val fusionEndpoints = "http://localhost:8765"
val queryProfilePath = s"/api/v1/query/best-buy"
...
spark.read.parquet(dataDir).limit(numQueries).rdd.repartition(numPartitions).foreachPartition(iter => {
    ...
    List.range(0,numUsersForThisQuery).foreach(n => {
      ...
      if (resp.isDefined) {
        ...
        if (random.nextDouble <= pctClicksPerVariant.apply(pctClickIdx)) {
          ...
          val fqi = rhParams.get("fusionQueryId")
          val fusionQueryId = if (fqi != null) fqi.asText() else randomUUID().toString
