...
// allow user to do "OR" boolean search
      var hasOR = q.match(/ OR /);
      if(hasOR){
        newQ = 'aero_title_t:(' + q + ')^25';
               newQ +=  ' OR aero_body_t:(' + q + ')^10';
              logger.info('query stage newQ =' + newQ);
  ...
  //allow users to do wildcard search
     var hasWildCard = q.match(/ .*\* /);
        if(hasWildCard){
          newQ = 'aero_title_t:' + q + '';
  ...
  //allow users to do tilde/proximity search
   var hasTilde = q.match(/.*(\.|!|~)$/);
        if(hasWildCard){
          newQ = 'aero_title_t:' + q + '';
                 newQ +=  ' OR aero_body_t:' + q + '';
                logger.info('query stage newQ =' + newQ);
